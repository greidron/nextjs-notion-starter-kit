...
export function contextUpdateToolUsage(context: AgentContext, toolName: string) {
    ...
    if (!context.info.toolUsage) {
        context.info.toolUsage = {}
    }
    if (context.info.toolUsage[toolName]) {
        context.info.toolUsage[toolName] = 0
    }
    ++context.info.toolUsage[toolName]
}

export function contextGetToolUsage(context: AgentContext): AgentToolUsage {
    return context?.info?.toolUsage || {}
}